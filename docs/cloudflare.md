...
6. **Token scopes.** Scopes of each Cloudflare token, and whether CI, the operator and any MCP client share one credential.
7. **Zone and redirect.** Whether `dicee.games` is a zone on this account, whether Registrar is used, and where the www-to-apex redirect lives (`packages/web/_redirects` says the dashboard). This feeds the organization move.
