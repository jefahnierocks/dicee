...
Near-term direction (status decision 9). Documentation, inventory planning and credential-free source validation can proceed now. Ownership changes and infrastructure adoption wait for exposed Cloudflare/Supabase token rotation, the Supabase key migration, HS256 removal and Infisical retirement (credentials before ownership). The active security runbook keeps its explicit stop points; this section does not advance an operator phase.
